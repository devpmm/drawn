...
export default function ConfigModal({ isOpen, onClose, onSave, initialConfig, showManager = false }) {
  ...
  // Only sync initial config into local form state to avoid overwriting user input on model load failure
  useEffect(() => {
    if (initialConfig) {
  ...
  // Determine whether to use custom model input based on available models
  useEffect(() => {
    if (config.model) {
      if (models.length > 0) {
  ...
  const handleLoadModels = async () => {
    if (!config.type || !config.baseUrl || !config.apiKey) {
      setError('Please fill in the provider type, base URL, and API key first');
      return;
    }
    ...
    try {
      ...
      if (data.models.length > 0) {
        // If the current model isn't in the new list, switch to list selection
        if (config.model && !data.models.some(m => m.id === config.model)) {
          ...
          // No model selected and not in manual entry mode — auto-select first
          setConfig(prev => ({ ...prev, model: data.models[0].id }));
        }
      }
    } catch (err) {
      ...
      <div className="relative bg-white rounded border border-gray-300 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="px-4 py-3 bg-blue-50 border border-blue-200 rounded">
            <p className="text-sm text-blue-800">Note: when an access password is enabled, the server-side configuration takes priority.</p>
          </div>
