...
export default function ConfigManager({ isOpen, onClose, onConfigSelect }) {
  ...
  const handleClone = (config) => {
    ...
    try {
      ...
      setError('Failed to duplicate config: ' + err.message);
    }
  };
      ...
      try {
        ...
        if (result.success) {
          setNotification({
            ...
            message: `${result.count} config(s) imported successfully`,
            type: 'success'
          });
          loadConfigs();
        } else {
      ...
      <div className="relative bg-white rounded border border-gray-300 w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div className="mb-4 px-4 py-3 bg-blue-50 border border-blue-200 rounded">
            <p className="text-sm text-blue-800">Note: when an access password is enabled, the server-side configuration takes priority and these configs are ignored.</p>
          </div>
          ...
          <div className="space-y-3">
            {filteredConfigs.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                {searchQuery ? 'No configs match your search' : 'No configs yet — click "New Config" to get started'}
              </div>
            ) : (
